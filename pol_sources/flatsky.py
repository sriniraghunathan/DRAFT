...
def make_gaussian_realisation(flatskymyapparams, el, cl, cl22 = None, cl12 = None, bl = None, qu_or_eb = 'eb'):
    ...
    """
    nx, ny, dx = flatskymyapparams
    arcmins2radians = np.radians(1/60.)

    dx = dy = dx * arcmins2radians
    ...
    #1d to 2d now
    cltwod = cl_to_cl2d(el, cl, flatskymyapparams)
    
    ################################################
    if cl22 is not None: #for TE, etc. where two fields are correlated.
        assert cl12 is not None
        cltwod12 = cl_to_cl2d(el, cl12, flatskymyapparams)
        cltwod22 = cl_to_cl2d(el, cl22, flatskymyapparams)
    ...
    if bl is not None:
        if np.ndim(bl) != 2:
            bl = cl_to_cl2d(el, bl, flatskymyapparams)
...
def gauss_beam(fwhm_am, lmax=512):
    import healpy as H
    """
    ...
    bl = H.gauss_beam(np.radians( fwhm_am/60.), lmax=lmax)
