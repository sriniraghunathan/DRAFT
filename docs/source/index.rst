...
The **DRAFT** (Dark Radiation Anisotropy Flowdown Team Tool) tool provides an
...
.. figure:: figures/pipeline.*
   :alt: Schematic overview of the DRAFT pipeline, from inputs through sky
         preparation, component separation and forecasting to outputs.
   :width: 100%

   Schematic overview of DRAFT and the pipeline it implements, from the
   instrumental and astrophysical inputs to the projected sensitivity to
   cosmological parameters. (Reproduced from |paper|.)
...
*[cross-ILC to be ported]*, leaving the residual power spectra
...
constraints on the cosmological parameters. The same formalism can also be used
to estimate the biases induced in those parameters by residual foregrounds
(§3.3 of |paper|).
*[A module driving both codes from within this repository is in preparation.]*
...
The component separation stage is already implemented in this repository, with
the two forecasting parts currently being implemented. The intermediate ILC
...
:doc:`installation` covers the requirements and how to run the driver,
:doc:`quickstart` the available arguments, :doc:`modules` the API organized by
pipeline stage, :doc:`products` the input data, the released data products and their file
format, and :doc:`references` the literature underlying the models and methods.
